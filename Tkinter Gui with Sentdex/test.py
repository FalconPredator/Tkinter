            ...
            if DataPace == "tick":
                ...
                if DatCounter > 12:
                    try:
                        if exchange == "Huobi":
                            ...
                            if topIndicator != "none" and bottomIndicator !="none":
                                ...
                                a3 = plt.subplot2grid((6,4), (5,0), sharex=a, rowspan=1, colspan=4)
                                 ...
                                 # main graph
                                a = plt.subplot2grid((4,1), (0,0), rowspan=3, colspan=6)
                                # volume
                                a2 = plt.subplot2grid((6,4), (5,0), sharex=a, rowspan=1, colspan=8)

                        ohlcData = spotAPI.get_kline('EOS-USDT', granularity=86400)
                        ...
                        OHLC.loc[:, ['open', 'high', 'low', 'close', 'volume']] = OHLC.loc[:, ['open', 'high', 'low', 'close', 'volume']] .astype(float)
                        ...
                        a.set_ylabel("Price")
                        if exchange != "Huobi":
                            a2.fill_between(OHLC["MPLDates"], 0, OHLC['volume'], facecolor=darkColor)
